...
router.post('/upload-csv', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        headers.forEach((h, idx) => {
            if (h.includes('date') || h.includes('दिनांक') || h.includes('time'))
                dateIdx = idx;
            else if (h.includes('desc') || h.includes('particular') || h.includes('detail') || h.includes('विवरण') || h.includes('narrative'))
                descIdx = idx;
            else if (h.includes('amount') || h.includes('debit') || h.includes('withdrawal') || h.includes('राशि') || h.includes('out') || h.includes('value'))
                amountIdx = idx;
            else if (h.includes('ref') || h.includes('txn') || h.includes('reference') || h.includes('id') || h.includes('संदर्भ'))
                refIdx = idx;
        });
        ...
        await prisma.$transaction(async (tx) => {
            for (let i = 1; i < lines.length; i++) {
                ...
                if (row.length < 3 || !row[dateIdx] || !row[amountIdx])
                    continue;
                ...
                if (rawAmount <= 0)
                    continue; // Skip deposit rows or invalid amounts
                // Determine AI category
                const category = await financial_service_1.FinancialService.categorizeExpense(rawDesc, rawAmount);
