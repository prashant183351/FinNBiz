...
export default function LoginPage() {
  const { login, user, companies, error, loading, clearError } = useAuth()
      ...
      <div className="w-full max-w-md p-8 m-4 bg-slate-900/40 border border-slate-800/80 rounded-3xl shadow-[0_0_50px_-12px_rgba(141,198,63,0.15)] backdrop-blur-xl z-10 transition-all duration-500 hover:border-brand-500/30">
        <div className="text-center mb-8 flex flex-col items-center">
          <div className="w-16 h-16 mb-4 flex items-center justify-center">
             <img src="/logo.png" alt="FinNBiz Logo" className="w-full h-full object-contain" />
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white">
            {t('brand.name')}
          </h1>
          <p className="text-sm text-brand-400 font-medium mt-2">{t('brand.tagline')}</p>
        </div>

        <div className="mb-6">
          <h2 className="text-2xl font-bold tracking-tight text-white">{t('auth.login.title')}</h2>
          <p className="text-xs text-slate-400 mt-1">{t('auth.login.subtitle')}</p>
        </div>
        ...
        {error && (
          <div className="p-3 mb-6 bg-red-950/40 border border-red-800/60 text-red-300 text-xs rounded-lg flex items-center gap-2 animate-shake">
            ...
            <span>{error === 'user_not_found' ? t('error.user_not_found') : t('error.login_failed')}</span>
          </div>
        )}
        ...
        <div className="mt-8 text-center text-xs text-slate-500 font-medium">
          ...
          <Link href="/register" className="text-brand-400 hover:text-brand-300 font-bold underline decoration-dotted transition-colors">
